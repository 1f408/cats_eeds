...
import (
	...
	"github.com/1f408/cats_eeds/md2html/dt_table/ast"
)
...
type dataTableASTTransformer struct {
}

var defaultDataTableASTTransformer = &dataTableASTTransformer{}

// NewTableASTTransformer returns a parser.ASTTransformer for tables.
func NewDataTableASTTransformer() parser.ASTTransformer {
	return defaultDataTableASTTransformer
}

func (a *dataTableASTTransformer) transformTable(n gast.Node) (gast.WalkStatus, error) {
	...
	for c := n.FirstChild() ; c != nil ; c = c.NextSibling() {
		if c.Kind() != ast.KindTableRow {
	 		continue
		}
		has_row = true

		cel := c.FirstChild()
		if cel.Kind() != ast.KindTableCell {
			all_em = false
			break
		}
		cel_d := cel.FirstChild()
		if cel_d == nil {
			break
		}
		if cel_d.Kind() != gast.KindEmphasis {
			all_em = false
			break
		}
	}
	...
	for c := n.FirstChild() ; c != nil ; c = c.NextSibling() {
		...
		if em.Kind() != gast.KindEmphasis {
			break
		}

		cel.RemoveChild(cel, em)
		for ec := em.FirstChild(); ec != nil ;{
			next := ec.NextSibling()

			em.RemoveChild(em, ec)
			cel.AppendChild(cel, ec)

			ec = next
		}
		cel.(*ast.TableCell).IsHeader = true
	}

	return gast.WalkSkipChildren, nil
}

func (a *dataTableASTTransformer) Transform(node *gast.Document, reader text.Reader, pc parser.Context) {
	ASTTWalk(node, a.transformTable)
}
...
func (r *TableHTMLRenderer) renderTableCell(
	...
	tag := "td"
	if n.Parent().Kind() == ast.KindTableHeader || n.IsHeader {
		tag = "th"
	}
	if entering {
...
func (e *table) Extend(m goldmark.Markdown) {
	m.Parser().AddOptions(
		...
		parser.WithASTTransformers(
			...
			util.Prioritized(defaultDataTableASTTransformer, 1),
		),
	)
