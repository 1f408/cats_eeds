...
import (
	...
	"github.com/1f408/cats_eeds/md2html/footnote/ast"
...
func NewFootnoteBlockParser(ctx *footnoteCtx) parser.BlockParser {
	return &footnoteBlockParser{Ctx: ctx}
}
...
func NewFootnoteParser(ctx *footnoteCtx) parser.InlineParser {
	return &footnoteParser{Ctx: ctx}
}
...
func NewFootnoteASTTransformer(ctx *footnoteCtx) parser.ASTTransformer {
	return &footnoteASTTransformer{Ctx: ctx}
}
...
func NewFootnoteHTMLRenderer(ctx *footnoteCtx) renderer.NodeRenderer {
...
func (r *FootnoteHTMLRenderer) renderFootnoteLink(
	...
	if entering {
		...
		_, _ = w.Write(r.getRefIndexId(node))
		_, _ = w.WriteString(`"><a href="#`)
		_, _ = w.Write(r.getIndexId(node))
		_, _ = w.WriteString(`" class="`)
		_, _ = w.Write(applyFootnoteTemplate(r.Ctx.Config.LinkClass,
			n.Index, n.RefCount))
		if len(r.Ctx.Config.LinkTitle) > 0 {
			_, _ = w.WriteString(`" title="`)
			_, _ = w.Write(util.EscapeHTML(applyFootnoteTemplate(r.Ctx.Config.LinkTitle, n.Index, n.RefCount)))
		}
		_, _ = w.WriteString(`" role="doc-noteref">`)
		_, _ = w.WriteString(r.index_str(node))
...
func (r *FootnoteHTMLRenderer) renderFootnoteBacklink(
	...
	if entering {
		...
		_, _ = w.Write(r.getRefIndexId(node))
		_, _ = w.WriteString(`" class="`)
		_, _ = w.Write(applyFootnoteTemplate(r.Ctx.Config.BacklinkClass, n.Index, n.RefCount))
		if len(r.Ctx.Config.BacklinkTitle) > 0 {
			_, _ = w.WriteString(`" title="`)
			_, _ = w.Write(util.EscapeHTML(applyFootnoteTemplate(r.Ctx.Config.BacklinkTitle, n.Index, n.RefCount)))
		}
		_, _ = w.WriteString(`" role="doc-backlink">`)
		_, _ = w.Write(applyFootnoteTemplate(r.Ctx.Config.BacklinkHTML, n.Index, n.RefCount))
		_, _ = w.WriteString(`</a>`)
	}
	return gast.WalkContinue, nil
}
...
func (r *FootnoteHTMLRenderer) index_str(node gast.Node) string {
	idx := 0
	if i, ok := ast.GetFootnoteIndex(node); ok {
		idx = i
	}
	return fmt.Sprintf("%d", idx)
}

func (r *FootnoteHTMLRenderer) getIndexId(node gast.Node) []byte {
	buf := []byte{}
	buf = append(buf, r.Ctx.IdPrefix...)

	idx := 0
	if i, ok := ast.GetFootnoteIndex(node); ok {
		idx = i
	}
	buf = fmt.Appendf(buf, "%d", idx)

	r.Ctx.IdsTable.Put(buf)
	return buf
}

func (r *FootnoteHTMLRenderer) getRefIndexId(node gast.Node) []byte {
	buf := []byte{}
	buf = append(buf, r.Ctx.IdPrefix...)
	...
	if ri, ok := ast.GetFootnoteRefIndex(node); ok && ri > 0 {
		buf = fmt.Appendf(buf, "-%d", ri)
	}
...
func applyFootnoteTemplate(b []byte, index, refCount int) []byte {
	...
	is := []byte(fmt.Sprintf("%d", index))
	rs := []byte(fmt.Sprintf("%d", refCount))
	ret := bytes.Replace(b, []byte("^^"), is, -1)
	return bytes.Replace(ret, []byte("%%"), rs, -1)
}

type cust_footnote struct {
	Ctx *footnoteCtx
}

type footnoteCtx struct {
	IdsTable uniqid.IdsTable
	IdPrefix []byte
	Config   *FootnoteConfig
}

func (ctx *footnoteCtx) TryStart() {
	if len(ctx.IdPrefix) == 0 {
		ctx.IdPrefix = getFootnotePrefix(ctx.IdsTable, ctx.Config.IDPrefix)
	}
}

func (ctx *footnoteCtx) Finish() {
	if len(ctx.IdPrefix) != 0 {
		ctx.IdsTable.CleanPrefix(ctx.IdPrefix)
		ctx.IdPrefix = []byte{}
	}
}

func getFootnotePrefix(ids_tbl uniqid.IdsTable, prepre []byte) []byte {
	if len(prepre) == 0 {
		prepre = []byte("fn")
	}
	prefix := make([]byte, 0, len(prepre)+3)
	prefix = append(prefix, prepre...)
	prefix = append(prefix, ':')

	if ids_tbl.IsValidPrefix(prefix) {
		ids_tbl.ReservePrefix(prefix)
		return prefix
	}

rewrite:
	for i := 1; ; i++ {
		prefix = prefix[:len(prepre)]
		prefix = fmt.Appendf(prefix, "-%d:", i)
		if ids_tbl.IsValidPrefix(prefix) {
			break rewrite
		}
	}
...
func NewFootnote(ids_tbl uniqid.IdsTable, opts ...FootnoteOption) goldmark.Extender {
	cfg := NewFootnoteConfig()
	for _, opt := range opts {
		opt.SetFootnoteOption(cfg)
	}
	ctx := &footnoteCtx{
		IdsTable: ids_tbl,
		IdPrefix: nil,
	...
	return &cust_footnote{
		Ctx: ctx,
	}
}

func (e *cust_footnote) Extend(m goldmark.Markdown) {
	m.Parser().AddOptions(
		parser.WithBlockParsers(
			util.Prioritized(NewFootnoteBlockParser(e.Ctx), 999),
		),
		parser.WithInlineParsers(
			util.Prioritized(NewFootnoteParser(e.Ctx), 101),
		),
		parser.WithASTTransformers(
			util.Prioritized(NewFootnoteASTTransformer(e.Ctx), 999),
		),
	)
	m.Renderer().AddOptions(renderer.WithNodeRenderers(
		util.Prioritized(NewFootnoteHTMLRenderer(e.Ctx), 500),
	))
